    ...
    try 
      ...
      |> Array.fold_left (fun acc name -> 
          ...
          if Sys.is_directory filepath then 
            acc 
          else if Str.string_match matcher name 0 then 
            filepath :: acc
          else acc ) [] |> fun acc -> Ok acc
    with Sys_error msg -> Error msg
  in
    walk path

let convert_plist_to_xml filepath =
  C_plist_xml.convert filepath

  let rec  string_of_plist_t = function
  | `Array lst -> List.fold_left (fun str v -> str ^ " " ^ string_of_plist_t v) "" lst
  | `Bool b -> string_of_bool b
  | `Data s -> s
  | `Date (f, opt) -> string_of_float f ^ " " ^ Option.value ~default:"None" (Option.map string_of_float opt)
